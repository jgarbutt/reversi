...
let username = decodeURIComponent(getIRIParameterValue("username"));
if (
...
let chatRoom = decodeURIComponent(getIRIParameterValue("game_id"));
if (
...
socket.on("game_start_response", (payload) => {
  ...
  window.location.href =
    "game.html?username=" + username + "&game_id=" + payload.game_id;
});
...
socket.on("join_room_response", (payload) => {
  ...
  */
  ...
  /* Announce in the chat that someone has arrived */
  let newHTML =
...
socket.on("game_over", (payload) => {
  ...
  let nodeD = $("<a href='lobby.html?username=" + username + "' class='btn-lg btn-primary' role='button'>Return to lobby</a>");
  nodeA.append(nodeB);
  nodeA.append(nodeC);
  nodeA.append(nodeD);
  nodeA.hide();
...
$(() => {
  ...
  request.username = username;
  console.log(
    "**** Client log message, sending 'join_room' command: " +
    JSON.stringify(request)
  );
